...
export default function Customize() {
  ...
  const [imageSource, setImageSource] = useState('reference');
  ...
  useEffect(() => {
    if (selectedModelId) {
      fetchCompatibleParts(selectedModelId);
      setSelectedParts([]);
      setResultImage(null);
      setPartSearch('');
      setPartCategory('all');
  ...
  async function fetchCompatibleParts(modelId) {
    ...
    if (!model) return;
    const modelLabel = `${model.make} ${model.model}`;
    const { data } = await supabase
      .from('parts')
      .select('*')
      .contains('compatible_models', [modelLabel]);
  ...
  async function handleGenerate() {
    setError('');
    if (!selectedModelId) { setError('Please select your motorcycle model first.'); return; }
    ...
    if (imageSource === 'upload' && !uploadedPhoto) {
      setError('Please upload a photo of your motorcycle, or switch to "Use our reference photo".');
      return;
    }
      ...
      const { data: fnData, error: fnError } = await supabase.functions.invoke(
        'generate-preview',
        {
          body: {
            ...
            motorcycleLabel: `${selectedModel.make} ${selectedModel.model}`,
            imageSource,
          },
        }
      );
      ...
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-400">
            Select your model, choose a photo, pick up to 3 parts, and generate a realistic AI preview.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-5">
            ...
            <div className="bg-dark-800 rounded-xl p-5">
              <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wide mb-3">
                1. Select Your Motorcycle Model
              </h2>
              <select
            ...
            {selectedModelId && (
              <div className="bg-dark-800 rounded-xl p-5">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <button type="button" onClick={() => setImageSource('reference')}
                    className={`rounded-lg p-3 text-sm font-medium border transition ${
                      imageSource === 'reference'
                        ? 'border-primary-500 bg-primary-500/10 text-primary-400'
                        : 'border-gray-700 text-gray-400 hover:border-gray-500'
                    }`}>
                    📷 Use Our Reference Photo
                  </button>
                ...
                {imageSource === 'reference' && selectedModel?.reference_photo_url && (
              ...
              <div className="bg-dark-800 rounded-xl p-5">
                ...
                {parts.length === 0 ? (
                  <p className="text-gray-400 text-sm">
                    No compatible parts found for {selectedModel?.make} {selectedModel?.model}.
                  </p>
                ) : (
                  <>
            ...
            <button onClick={handleGenerate}
              disabled={loading || !selectedModelId || selectedParts.length === 0}
          ...
          <div className="bg-dark-800 rounded-xl p-5 flex flex-col">
            ...
            <p className="text-xs text-gray-500 mb-4">
              AI-generated appearance of your motorcycle with selected parts applied.
            </p>
              ...
              {loading ? (
                ...
                <div className="text-center px-6">
                  ...
                  <p className="text-gray-600 text-xs mt-2">
                    Select a model, photo source, and up to 3 parts to get started.
                  </p>
                </div>
              )}
            </div>
